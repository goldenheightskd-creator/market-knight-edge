...
import { Calculator, ShieldCheck, Clock, BookOpen, GraduationCap, Bell } from "lucide-react";
...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-50 border-t border-border bg-background/95 backdrop-blur-md">
      <div className="mx-auto max-w-2xl grid grid-cols-6 px-1 pt-1 pb-[max(env(safe-area-inset-bottom),8px)]">
        {tabs.map(({ to, label, Icon }) => {
          ...
          return (
            <Link key={to} to={to} className="group flex flex-col items-center justify-center gap-1 py-1.5 text-[9px] font-semibold tracking-wide uppercase">
              <div className={`h-8 w-10 grid place-items-center rounded-lg transition-colors ${active ? "bg-accent/15 text-accent" : "text-muted-foreground"}`}>
